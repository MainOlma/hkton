...
var _gridmap = _interopRequireDefault(require("./gridmap.csv"));
...
function getColor(value, max) {
  var color = d3.scaleSequential(d3.interpolateGreens).domain([-max * 0.5, max * 1.5]);
...
function getColorField(value, max, min) {
  /*const color = d3.scaleSequential(d3.interpolateGreens)
      .domain([0, max]);*/
  var color = d3.scaleLinear().domain([-10, 0, 10]).range(['#ff7373', '#5e9cfa', '#ff7373']).clamp(true); //.interpolate(d3.interpolateHcl);
...
function updateMap(subjects_data, map_data) {
  ...
  console.log(subjects_data.filter(function (d) {
    return d.service_type == service_type;
  }).map(function (d) {
    return {
      val: +d[targetField],
  ...
  var mainSvg = d3.select("#root svg.mainChart");
  var rects = mainSvg.selectAll("rect").data(map_data.map(function (d) {
  ...
  rects.exit() //.attr("class", "exit sub")
  .transition(t).remove();
  rects
  /*.attr("class", "update sub")*/
  ...
  texts.style("opacity", 0).transition(t2).style("opacity", 1).text(function (d) {
    //console.log(d)
    var context;
    if (!Number.isNaN(d.value) && d.value != '') context = f(d.value);else context = "—";
    return context;
  });
}
...
},{"d3":"../node_modules/d3/index.js","./gridmap.csv":"gridmap.csv","./data.csv":"data.csv"}],"../../../../../usr/local/lib/node_modules/parcel-bundler/src/builtins/hmr-runtime.js":[function(require,module,exports) {
